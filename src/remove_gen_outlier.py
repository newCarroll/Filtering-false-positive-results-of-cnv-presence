...
def print_regions_plots():
    for file in file_genome_coverage.keys():
        prev_gen = region_list[0][3]
        # список покртыий регионов гена
        region_coverages = []
        # текушие регионы одного гена
        current_regions = []
...
def find_gen_neighbors(coverages1, coverages2, any_file):
    # из file_regions. В нем списоег больших регионов состоящи й из спсика маленьких регионов
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        find_gen_neighbors(rates_regions_file0, rates_regions_file1, files[0])
